...
class RolloutStorage:
    class Transition:
        def __init__(self) -> None:
            ...
            # Per-transition validity mask for inline-settling. Shape
            # (num_envs,) or (num_envs, 1); None means "treat as 1.0". A
            # settling-mode env's transition is stored but written with
            # valid_mask=0 so it contributes no gradient signal.
    ...
    def __init__(
        ...
        if training_type == "rl":
            ...
            # Validity mask, default 1.0 → behaves like "no mask" unless
            # overwritten per-step by a caller that's tracking settling envs.
            self.valid_masks = torch.ones(num_transitions_per_env, num_envs, 1, device=self.device)
    ...
    def add_transitions(self, transition: Transition) -> None:
        ...
        if self.training_type == "rl":
            ...
            # Validity mask: default 1.0 when caller doesn't supply one, so
            # callers that don't opt into inline-settling are unaffected.
            if transition.valid_mask is not None:
                self.valid_masks[self.step].copy_(transition.valid_mask.view(-1, 1).to(self.valid_masks.dtype))
            else:
                self.valid_masks[self.step].fill_(1.0)
    ...
    def _save_hidden_states(self, hidden_states: tuple[HiddenState, HiddenState]) -> None:
        ...
        hidden_state_c = hidden_states[1] if hidden_states[1] is not None and isinstance(hidden_states[1], tuple) else (hidden_states[1],) if hidden_states[1] is not None else None
    ...
    def compute_returns(
        ...
        bleed_guard: bool = True,
    ) -> None:
        """GAE with inline-settling aware masking.

        When ``bleed_guard`` is True (default), invalid-mask transitions (a)
        contribute zero δ and (b) break the recursive advantage chain — a
        masked step behaves like a "wall" preventing reward/advantage from a
        valid transition leaking through a settling one to a different valid
        transition. When masks are all-ones, this is a bit-exact no-op.

        Advantage normalization is also mask-aware: stats are computed over
        the valid subset only so masked zeros don't skew μ/σ.
        """
        advantage = 0
        use_mask = bleed_guard and hasattr(self, "valid_masks")
        for step in reversed(range(self.num_transitions_per_env)):
            ...
            if use_mask:
                mask_step = self.valid_masks[step]
                # Zero out invalid transitions so they contribute no gradient.
                advantage = advantage * mask_step
        ...
        if normalize_advantage:
            if use_mask and not bool((self.valid_masks == 1.0).all().item()):
                # Mask-aware stats: only valid transitions contribute to μ/σ.
                # Fall through to the fast path when every mask is 1, so
                # Stage-2 behavior stays numerically identical to baseline.
                m = self.valid_masks
                count = m.sum().clamp_min(1.0)
                adv_mean = (self.advantages * m).sum() / count
                var = ((self.advantages - adv_mean).pow(2) * m).sum() / count
                adv_std = var.clamp_min(1e-16).sqrt()
    ...
    def mini_batch_generator(self, num_mini_batches: int, num_epochs: int = 8) -> Generator:
        ...
        valid_masks_flat = self.valid_masks.flatten(0, 1)
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                valid_mask_batch = valid_masks_flat[batch_idx]

                hidden_state_a_batch = None
                hidden_state_c_batch = None
                masks_batch = None

                # Yield the mini-batch — valid_mask_batch appended last so the
                # existing 10-tuple callers upgrade by unpacking 11 elements.
                yield (
                    obs_batch,
                    actions_batch,
                    target_values_batch,
    ...
    def recurrent_mini_batch_generator(self, num_mini_batches: int, num_epochs: int = 8) -> Generator:
        ...
        for ep in range(num_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                valid_mask_batch = self.valid_masks[:, start:stop]
                ...
                if hidden_state_c_batch is not None:
                    hidden_state_c_batch = (
                        hidden_state_c_batch[0] if len(hidden_state_c_batch) == 1 else hidden_state_c_batch
                    )

                # Yield the mini-batch (recurrent path); valid_mask appended
                # last to match the feedforward contract.
                yield (
                    obs_batch,
                    actions_batch,
                    values_batch,
